...
As the code is run, the **ANN** will improve its accuracy after every iteration (called **epoch**). Once the network reaches a certain degree of accuracy, the program will print out a graph of Loss/Accuracy vs. Iterations. If that graph is closed, another
